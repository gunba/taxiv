    ...
    def get(self, text_chunk, model_name):
        ...
        with self.lock:
            try:
                ...
                cursor = conn.cursor()
                cursor.execute("SELECT response_json, input_tokens, output_tokens FROM cache WHERE hash = ? AND model_name = ?", (hash_key, model_name))
...
def initialize_gemini_client():
    ...
    if not api_key:
        tqdm.write(f"Warning: Gemini API Key not found in environment variable '{API_KEY_ENV_VAR}'. Proceeding without LLM features.")
        return
