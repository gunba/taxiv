...
class RelatednessIndexerConfig:
    ...
    # Text chunking for long provisions/documents
    # Average normalized chunk embeddings to one provision vector
    chars_per_chunk: int = int(os.getenv("RELATEDNESS_EMBED_CHARS_PER_CHUNK", "1200"))
    chunk_overlap: int = int(os.getenv("RELATEDNESS_EMBED_CHUNK_OVERLAP", "200"))
    max_chunks: int = int(os.getenv("RELATEDNESS_EMBED_MAX_CHUNKS", "16"))  # up to ~19k chars by default
...
def _chunk_text(text: str, chars_per_chunk: int, overlap: int, max_chunks: int) -> List[str]:
    """Greedy character-based chunking with overlap. Works well enough for ST models."""
    if not text:
        return []
    n = max(1, chars_per_chunk)
    ov = max(0, min(overlap, n // 2))
    chunks: List[str] = []
    i = 0
...
def _prep_text_for_embedding(p: dict, cfg: RelatednessIndexerConfig) -> List[str]:
    title = (p.get("title") or "").strip()
    body = (p.get("content_md") or "").strip()
    txt = f"{title}\n{body}".strip()
    # For very long content, chunk without an arbitrary global cap
    return _chunk_text(txt, cfg.chars_per_chunk, cfg.chunk_overlap, cfg.max_chunks)
...
def upsert_provision_embeddings(
        ...
        batch_size: int = 64,
        max_text_chars: int = 0,  # deprecated; ignored in favor of chunking
):
    """Compute embeddings for new/updated provisions and upsert into pgvector.

    This version removes the old 4k clamp. We chunk long texts and mean-pool the
    normalized chunk embeddings into one provision vector.
    """
    if SentenceTransformer is None or np is None:
        logger.warning("Embedding model not available. Skipping embeddings upsert.")
        return

    cfg = RelatednessIndexerConfig()  # ensure we use the same defaults as runtime
    provision_ids = [p["internal_id"] for p in provisions_payload]
    all_chunks: List[str] = []
    chunk_owner: List[int] = []  # index of provision_ids each chunk belongs to

    for idx, p in enumerate(provisions_payload):
        chunks = _prep_text_for_embedding(p, cfg)
        if not chunks:
            chunks = [p.get("title") or p.get("content_md") or ""]
        start = len(all_chunks)
        all_chunks.extend(chunks)
        chunk_owner.extend([idx] * len(chunks))
        # Guarantee at least one chunk entry per provision
        if len(all_chunks) == start:
            all_chunks.append("")

    logger.info("Encoding %d chunks for %d provisions with %s", len(all_chunks), len(provision_ids), model_name)
    ...
    # Pool chunk vectors per provision (mean of normalized vectors -> renormalize)
    dim = int(vectors.shape[1]) if hasattr(vectors, "shape") else len(vectors[0])
    pooled: List[np.ndarray] = [np.zeros(dim, dtype=np.float32) for _ in provisions_payload]
    counts: List[int] = [0] * len(provisions_payload)

    for vec, owner_idx in zip(vectors, chunk_owner):
        pooled[owner_idx] += vec
        counts[owner_idx] += 1

    for i in range(len(pooled)):
        if counts[i] > 0:
            pooled[i] /= float(counts[i])
        # L2 normalize pooled vector
        norm = np.linalg.norm(pooled[i]) or 1.0
        pooled[i] = (pooled[i] / norm).astype(np.float32)
    ...
    try:
        ...
        for pid, vec in zip(provision_ids, pooled):
            db.merge(Embedding(
