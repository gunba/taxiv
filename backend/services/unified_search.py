...
# Penalize definitions from dominating lists
DEF_RESULT_PENALTY = 0.75
...
def _lexical_candidates(db: Session, original: str, normalized: str, limit: int = LEX_TOP) -> Dict[str, float]:
	...
	sql = text(
		...
		SELECT
		  ...
		  -- Rank component
		  GREATEST(
		    ts_rank_cd(b.tsv_en, websearch_to_tsquery('english', :q_norm)),
		    ts_rank_cd(b.tsv_simple, websearch_to_tsquery('simple', :q_norm))
		  ) AS ts_score,
		  -- Trigram max against raw and normalized, over title and content
		  GREATEST(
		    similarity(lower((SELECT title FROM provisions p2 WHERE p2.internal_id=b.internal_id)), lower(:q_norm)),
		    similarity(lower((SELECT coalesce(content_md,'') FROM provisions p2 WHERE p2.internal_id=b.internal_id)), lower(:q_norm)),
		    similarity(lower((SELECT title FROM provisions p2 WHERE p2.internal_id=b.internal_id)), lower(:q_raw)),
		    similarity(lower((SELECT coalesce(content_md,'') FROM provisions p2 WHERE p2.internal_id=b.internal_id)), lower(:q_raw))
		  ) AS tri_score
		FROM base b
		ORDER BY
		  (GREATEST(
...
def unified_search(db: Session, query: str, k: int = 25) -> dict:
	...
	for did in interpretation["definitions"]:
		seed_weights[did] = seed_weights.get(did, 0.0) + 0.75  # lighter than provisions
	...
	if not seed_weights and lex_candidates:
		# Normalize lexical scores into weights
		if lex_candidates:
			vals = list(lex_candidates.values())
			m_scaled = _minmax_scale(vals)  # 0..100
			total = sum(m_scaled.values()) or 1.0
			for i, iid in enumerate(lex_candidates.keys()):
				w = (m_scaled.get(i, 0.0) / total)
				if w > 0:
					seed_weights[iid] = seed_weights.get(iid, 0.0) + w
	...
	for seed_id, weight in seed_weights.items():
		...
		# include the seed itself with a small mass so exact matches surface
