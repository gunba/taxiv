...
const InteractiveContent: React.FC<InteractiveContentProps> = ({node, onTermClick, onReferenceByRefIdClick}) => {
    ...
    const CustomImage = ({alt, src, title, ...rest}: React.ImgHTMLAttributes<HTMLImageElement>) => {
        return (
            <img
                alt={alt ?? ''}
                src={src ?? ''}
                ...
                loading="lazy"
                decoding="async"
                className="mx-auto my-4 max-h-[28rem] w-auto rounded-md border border-gray-700 bg-gray-900 object-contain"
                {...rest}
            />
        );
    };
