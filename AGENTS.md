...
3. When the user defines a task or request, you must first create a plan and submit it to the user for approval. The template for the plan is available in `agents/plan.md`.
	a. **When drafting a plan, ensure that the plan has sufficient technical detail that another agent with no additional context could follow it**. 
...
6. Before the task is returned to the user
